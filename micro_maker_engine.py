...
class MicroMakerEngine:
    def __init__(self, store: ConfigStore, notify: Notify | None = None):
        ...
        # v0057: market signal hold/stability guard. A one-tick acceleration spike
        ...
        # v0057: optional TOP10 leader signal mode. Leaders decide only market
        ...
        # v0057 Mirror Lab: virtual-only replay buffer. No real orders are sent.
    ...
    def _blocked_symbol(symbol: str) -> bool:
        ...
        if "STOCK" in sym:
            return True
        # v0057: the account balance/margin shown by MEXC is USDT. Contracts like
    ...
    async def _ensure_market_ws(self, symbols: list[str], s: dict[str, Any]) -> None:
        ...
        v0057 rule: 0 means ALL. The price vote must be based on the real
    ...
    async def start(self) -> str:
        ...
        return "▶️ Price Tsunami v0057 запущен. Схема: price-scan 10s по активным zero-fee монетам → LONG/SHORT/NEUTRAL проценты → 5 сделок одной стороной → закрытие всей корзины по REAL NET PnL."
    ...
    def quick_status_text(self) -> str:
        ...
        v0057 UI rule: market scan blocks are ALWAYS visible, even when the
    ...
    async def scan_now_text(self) -> str:
        ...
        try:
            ...
            # v0057: scan_now must be read-only for signal/hold state; pressing the
            ...
            return (
                f"🔍 Price Scan {s.get('bot_version', 'v0057')}\n"
    ...
    async def _run_loop(self) -> None:
        ...
        await self._notify("✅ Price Tsunami v0057 started: ALL mode 65/75% +15п.п.; TOP10 mode 7/10 normal, 7/10 +2 early, 8/10 tsunami; HOLD 4/5 за ~10s; entries from full zero-fee universe.")
        while self.running:
            try:
                s = self._settings()
    ...
    async def _symbol_pool(self, s: dict[str, Any]) -> list[str]:
        ...
        def apply_scan_cap(pool: list[str]) -> list[str]:
            # v0057: max_zero_fee_scan_symbols <= 0 means ALL symbols, no 250 cap.
    ...
    async def _refresh_market_scan(self, s: dict[str, Any], force: bool = False) -> list[dict[str, Any]]:
        ...
        for sym in pool:
            try:
                ...
                # v0057: Price Tsunami no longer uses the old order-book
                ...
                # v0057: score is no longer the market-direction source. Keep it only
                ...
                scored.append({
                    ...
                    "score": score,
                    "vote": vote,
                    # v0057: in Price Tsunami the trade side comes from the 10s
        ...
        # v0057: denominator must be the whole selected universe. If a symbol has
        ...
        # v0057: precompute TOP10 leaders from the same full zero-fee pool.
        ...
        # v0057 Mirror Lab records only compact virtual snapshots. It does not
    ...
    def _recent_move_pct(self, symbol: str, lookback_sec: float) -> tuple[float | None, float]:
        ...
        v0057 Wave Price Tsunami Basket deliberately uses this simple fact instead of
    ...
    async def _choose_direction(self, symbol: str, s: dict[str, Any], book: dict[str, Any]) -> str | None:
        ...
        # v0057 Wave Price Tsunami Basket: market direction is not taken from the old
    ...
    def mirror_lab_report_text(self) -> str:
        ...
        if len(snaps) < min_snaps:
            return (
                f"🪞 Mirror Lab {s.get('bot_version', 'v0057')}\n"
    ...
    def _detect_wave_signal(self, rows: list[dict[str, Any]], s: dict[str, Any]) -> tuple[str | None, list[dict[str, Any]], dict[str, Any]]:
        """Detect v0057 Price Tsunami from the selected market signal source.
        ...
        if signal_mode == "top10_leaders":
            ...
            # v0057: TOP10 is mapped to the ALL-zero logic:
        ...
        # v0057 HOLD rule: +15p.p. must be stable, not a one-scan spike.
    ...
    async def _open_wave_position(self, symbol: str, direction: str, s: dict[str, Any], equity_before: float | None) -> dict[str, Any] | None:
        ...
        if margin_usdt <= 0:
            ...
            return None
        # v0057: aggressive entry means: choose a price that already exists in
    ...
    async def _wave_basket_cycle(self, side: str, rows: list[dict[str, Any]], signal: dict[str, Any] | None = None) -> None:
        ...
        # v0057: the first 5 picks can become stale while the controlled burst is
        ...
        # v0057: if MEXC charged real fees despite the zero-fee universe, do not
    ...
    async def _manage_basket_position(self, symbol: str, direction: str, pos: dict[str, Any], s: dict[str, Any], equity_before: float | None = None) -> None:
        """v0057 Wave Price Tsunami Basket manager.
        ...
        while self.running:
            ...
            # v0057 rotation: after the stale timeout, stop waiting for +$0.01.
